...
if -%1-==-- (
    ...
    for %%i in (*) do if not "%%~i" == "generate.bat" del "%%~i"
    for /d %%i in (*) do if not "%%~i" == "generate.bat" (
        del /f /s /q "%%~i" 1>nul
        rmdir /s /q "%%~i" 1>nul
    )
    exit
...
:help
    echo Batch file to generate project files for building SVT-AV1 on Windows
    echo Usage: generate.bat 2019^|2017^|2015^|clean
    exit
goto :EOF