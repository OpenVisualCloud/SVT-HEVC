...
| **MaxCLL** | -max-cll | [0 , 2^16-1] | 0 | Enables or disables the VPS timing info, 0 = OFF, 1 = ON | Maximum content light level (MaxCLL) as required by the Consumer Electronics Association 861.3 specification. Applicable for HDR content. If specified, signalled only when HighDynamicRangeInput is set to 1 |
