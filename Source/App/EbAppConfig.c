...
#define MAX_LCU_SIZE                                64
    ...
    if (config->tileRowCount < 1 || config->tileRowCount > EB_TILE_ROW_MAX_COUNT) {
        printf("Error Instance %u: Invalid TileRowCount. TileRowCount range should be 1 to 32. In order to specify more than 32 tile rows, please increase the value of the macro EB_TILE_ROW_MAX_COUNT defined in EbDefinitions.h\n", channelNumber + 1);
        return_error = EB_ErrorBadParameter;
    }

    if ((config->tileColumnCount * config->tileRowCount) <= 1 && config->tileSliceMode) {
        printf("Error Instance %u: Invalid TileSliceMode. TileSliceMode is 0 or 1, only could be 1 for multi tile mode, please set it to 0 for single tile mode\n", channelNumber + 1);
        return_error = EB_ErrorBadParameter;
    }
