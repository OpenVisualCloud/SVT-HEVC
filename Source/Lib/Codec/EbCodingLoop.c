    ...
    EncDecContext_t         *contextPtr)
{
    ...
    EB_BOOL useDeltaQp = (EB_BOOL)(sequenceControlSetPtr->staticConfig.improveSharpness || sequenceControlSetPtr->staticConfig.bitRateReduction ||(sequenceControlSetPtr->staticConfig.vbvBufsize && sequenceControlSetPtr->staticConfig.vbvMaxrate));
    ...
    EB_BOOL useDeltaQpSegments = singleSegment ? 0 : (EB_BOOL)(sequenceControlSetPtr->staticConfig.improveSharpness || sequenceControlSetPtr->staticConfig.bitRateReduction||(sequenceControlSetPtr->staticConfig.vbvBufsize && sequenceControlSetPtr->staticConfig.vbvMaxrate));
    ...
    while (cuItr < CU_MAX_COUNT) {
        if (codedLeafArrayPtr[cuItr]->splitFlag == EB_FALSE){
			...
			cuPtr->qp = (sequenceControlSetPtr->staticConfig.improveSharpness || sequenceControlSetPtr->staticConfig.bitRateReduction) && !(sequenceControlSetPtr->staticConfig.vbvBufsize && sequenceControlSetPtr->staticConfig.vbvMaxrate) ? 
                contextPtr->qpmQp : lcuPtr->qp;
			lcuPtr->qp = (sequenceControlSetPtr->staticConfig.improveSharpness || sequenceControlSetPtr->staticConfig.bitRateReduction) && !(sequenceControlSetPtr->staticConfig.vbvBufsize && sequenceControlSetPtr->staticConfig.vbvMaxrate) ?
                contextPtr->qpmQp : lcuPtr->qp;
			cuPtr->orgDeltaQp = cuPtr->deltaQp;
            ...
            if (cuPtr->predictionModeFlag == INTRA_MODE &&
				...
				for (partitionIndex = 0; partitionIndex < 4; partitionIndex++) {
                    ...
                    EB_BOOL pictureLeftBoundary =
                        (lcuPtr->pictureLeftEdgeFlag == EB_TRUE && ((partitionOriginX & (MAX_LCU_SIZE - 1)) == 0)) ? EB_TRUE : EB_FALSE;
                    EB_BOOL pictureTopBoundary =
                        (lcuPtr->pictureTopEdgeFlag == EB_TRUE && ((partitionOriginY & (MAX_LCU_SIZE - 1)) == 0)) ? EB_TRUE : EB_FALSE;
                    EB_BOOL pictureRightBoundary =
                        (lcuPtr->pictureRightEdgeFlag == EB_TRUE && (((partitionOriginX + MIN_PU_SIZE) & (MAX_LCU_SIZE - 1)) == 0)) ? EB_TRUE : EB_FALSE;
